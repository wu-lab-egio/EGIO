...
if sys.platform == 'darwin':
    align= ctypes.cdll.LoadLibrary(os.getcwd() + '/___pairwisealign_macos.so')
if sys.platform == 'linux':
    align= ctypes.cdll.LoadLibrary(os.getcwd() + '/___pairwisealign_linux.so')
...
def DFtoDict(dataframe, key_base, header=True):
    ...
    if header:
        if not(key_base in colname):
            SyntaxError("no column name called: " + key_base)

        keyi = listwhich(colname,"==",key_base)[0]
    else:
        keyi = key_base
...
def pairwisealign(seqref,seqtst,method,gapopen=-10):
...
def calIdentity(seqref, seqtst, method, gapopen=-10):
...
if __name__ == '__main__':
    ...
    print("organize ortholgous exon and isoform data")
    ...
    for res in res_l:
        datatmp = res.get()
        if len(datatmp["oe"]["dat"]) > 0:
            exoniden = exoniden + datatmp["oe"]["dat"]
        if len(datatmp["ot"]["dat"]) > 0:

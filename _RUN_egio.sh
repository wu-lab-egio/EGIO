...
# 1. BLAST+ program [https://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/LATEST/]
# 2. python packages: pandas, numpy


# cd /Users/jeffma/MJFPhD/HsSpDB/step0_GEIO_script/testprocess/
...
###=======================================
### make data base
echo "generate blast databases of $species1 and $species2"

makeblastdb -in $species1.exonfasta -dbtype nucl -out $species1/$species1
makeblastdb -in $species2.exonfasta -dbtype nucl -out $species2/$species2
...
blastn -query $species1.exonfasta -out $species1-$species2 -db $species2/$species2 -outfmt 6 -evalue 1e-5 -num_threads $pnum

echo "reciprocal blastn of $species1 and $species2: $species2 as query"

blastn -query $species2.exonfasta -out $species2-$species1 -db $species1/$species1 -outfmt 6 -evalue 1e-5 -num_threads $pnum

###=======================================
### summarize reciprocal blast
printf "summarize reciprocal blast"

python __prepare_egio_blastn.py --species1 $species1 --blast1 $species1-$species2 --exonanno1 $species1.exon --species2 $species2 --blast2 $species2-$species1  --exonanno2 $species2.exon --orthogen $ortholog

###=======================================
### run EGIO
echo "detect orthologous exons and isoforms between species $species1 and $species2"
python __EGIO.py --orthog $ortholog --blastn blastn_mapping.tab --species1 $species1 --species2 $species2 --isocom1 $species1.tran --cdscom1 $species1.exon --isocom2 $species2.tran --cdscom2 $species2.exon --identhres $identity --coverthres $coverage --match $match --mismatch $mismatch --gap $gap --pnum $pnum
###=======================================

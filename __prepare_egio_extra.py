...
def prepare_geio_extra(gtf, cdna, cds, species, outpath):
    if not(outpath == None):
        outpath = str(outpath)
    ...
    #outtsqlpath = str(outpath) + "/" + species + "_traninfo.tab"
    #hstcom = pd.DataFrame(hst)
    #hstcom.columns = 
    #hstcom.to_csv(outtsqlpath,index=False,sep='\t')

    outtsqlpath = str(outpath) + "/" + species + ".tran"
    ...
    ##===============================================
    #print("write exon sequence")
    #outpathCDS = str(outpath) + "/" + species + "_exon_seqinfo.tab"
    #CDSdf = pd.DataFrame(CDSinfostore)
    #CDSdf.columns = ["EnsemblG","ID","Chrom","Start","End","Strand","Seq"]
    #CDSdf.to_csv(outpathCDS,index=False,sep='\t')

    outpathCDS = str(outpath) + "/" + species + ".exon"
    ...
    outtfapath = str(outpath) + "/" + species + ".exonfasta"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--outpath',
        nargs='?',
        default=None,
        help='path to write summarized information')

    args = parser.parse_args()
    prepare_geio_extra(args.gtf, args.cdna, args.cds, args.species, args.outpath)

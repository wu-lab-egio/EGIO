...
def testquery(speciespair, orthologpair, feature, extrapath, egpath):
    #wdpath = os.getcwd()
    if extrapath[-1] != "/":
        extrapath = extrapath + "/"

    ##===============================================
    infiles = os.listdir(extrapath)
    ##===============================================
    ...
    exon1 = species1 + "." + feature
    exon2 = species2 + "." + feature

    egfile = "ExonGroup_testpro_"+species1+"_"+species2+"_"+feature+".txt"
    ...
    ##===============================================
    egfile = pd.read_table(egpath,header=0, sep="\t")

    genelist1 = list(egfile[species1+"Gene"])
    genelist2 = list(egfile[species2+"Gene"])
    ...
    ##===============================================
    tranfile1 = pd.read_table(extrapath+tran1,header=0, sep="\t")
    tranfile2 = pd.read_table(extrapath+tran2,header=0, sep="\t")

    exonfile1 = pd.read_table(extrapath+exon1,header=0, sep="\t")
    exonfile2 = pd.read_table(extrapath+exon2,header=0, sep="\t")    
    ##===============================================
...
## AsDataFrame: 
def aslsls(datastr,feature,typename="isoinfo/codon/cds/motif/exong"):
    ...
    if typename == "isoinfo":
        ...
        for i in range(0,len(datainfo)):
            if feature == "cds" and datainfo[i][orfi] != ".|.":
...
# genestd=geninfohs;regidxd=cdshs
...
# isoalid=isoinfohs; gensumd=geninfohs; species = "ctr"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--feature',
        nargs='?',
        help='feature: cds or exon')
    parser.add_argument('--extrapath',
        nargs='?',
        help='path to files generated by __prepare_egio_extra.py')
    parser.add_argument('--egpath',
        nargs='?',
        help='path to EG group mapping file')
    ...
    test = testquery(speciespair, orthologpair, args.feature, args.extrapath, args.egpath)
    ##-------------------------------------------
    geninfohs = aslsls(test['genestr1'], feature = args.feature, typename="isoinfo")
    cdshs = aslsls(test['cdsstr1'], feature = args.feature, typename="cds")
    geninfott = aslsls(test['genestr2'], feature = args.feature, typename="isoinfo")
    cdstt = aslsls(test['cdsstr2'], feature = args.feature, typename="cds")

    strandhs = selectcol(geninfohs,"strand")[0]
    strandtt = selectcol(geninfott,"strand")[0]

    #codon = aslsls(codonstr, feature = args.feature, typename="codon")
    exongroup = aslsls(test['egstr'], feature = args.feature, typename="exong")
    ...
    if args.outpath[-1] == "/":
        p.save(args.outpath + orthologpair+"."+args.feature+".pdf", width=6.5, height=canvas_y)
    else:
        p.save(args.outpath + "/" + orthologpair+"."+args.feature+".pdf", width=6.5, height=canvas_y)

# python __plotIsoCom.py --speciespair ctr-tdp --orthologpair ENSG00000116688-ENSG00000116688 --feature cds --extrapath /Users/jeffma/EGIO-main/extrainfo --egpath /Users/jeffma/EGIO-main/ExonGroup_testpro_ctr_tdp_cds.txt --outpath /Users/jeffma/EGIO-main/plot
# python __plotIsoCom.py --speciespair ctr-tdp --orthologpair ENSG00000116688-ENSG00000116688 --feature exon --extrapath /Users/jeffma/EGIO-main/extrainfo --egpath /Users/jeffma/EGIO-main/ExonGroup_testpro_ctr_tdp_exon.txt --outpath /Users/jeffma/EGIO-main/plot
